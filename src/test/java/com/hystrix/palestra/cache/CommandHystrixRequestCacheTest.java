...
public class CommandHystrixRequestCacheTest {
	...
	public void shouldGetValueFromCache() throws Exception {
		...
		try {
			assertThat(new PaymentCommandRequestCache(10).execute(), equalTo(true));
			assertThat(new PaymentCommandRequestCache(16).execute(), equalTo(true));
			assertThat(new PaymentCommandRequestCache(20).execute(), equalTo(true));
	...
	public void shouldGetValueFromCacheAndIndicateItFromHystrix() throws Exception {
		HystrixRequestContext context = HystrixRequestContext.initializeContext();
		try {
			HystrixCommand<Boolean> firstCommand = new PaymentCommandRequestCache(10);

			assertThat(firstCommand.execute(), equalTo(true));
			assertThat(firstCommand.isResponseFromCache(), equalTo(false));

			HystrixCommand<Boolean> secondCommand = new PaymentCommandRequestCache(10);
	...
	public void shouldGetValueFromCacheAndIndicateItFromHystrixButAnotherContext() throws Exception {
		...
		try {
			...
			HystrixCommand<Boolean> thirdCommand = new PaymentCommandRequestCache(10);
